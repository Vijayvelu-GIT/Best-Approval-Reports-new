...
async function getGeneralPoApproval(data, res) {
  ...
  try {
    ...
    // EX.COMPANYID ENAME,A.DOCID, A.DOCDATE,'GENPOMAS' TNAME,A.CREATEDBY,C.PARTYID,'Purchase Order.' SNAME,
    // 't'||A.TSID TRANSID, A.TOTQTY, A.APPLEVEL + 1 APPLEV,A.NET,D.TRIMNAME,A.DOCMAXNO,A.FINYEAR,
    ...
    // WHERE A.COMP = EX.COMPMASID AND A.CANCEL = 'F' AND A.PARTYID = C.PARTYMASID AND A.GENPOMASID = D.GENPOMASID 
    // AND A.REQUEST = 'T' AND EX.COMPANYID=E.COMPANYID AND ((A.APPLEVEL + 1 =E.APPLEVEL AND A.STATUS = 'Requested') OR 
    // (A.STATUS = 'Rejected' AND A.APPLEVEL =E.APPLEVEL) )) A, (SELECT   B.COMPANYID ENAME, A.TSNAME, A.MAXLEVEL FROM   
    // APPRMAS A, COMPMAS B WHERE A.ENAME = B.COMPMASID AND TSNAME = 'Purchase Order New.') B WHERE A.ENAME=B.ENAME
    // ORDER BY   A.ENAME, FINYEAR, DOCMAXNO`

    sql = `SELECT A.*,'Single' APPTYPE,NVL(MAXLEVEL, 0) MAXLEVEL FROM (SELECT A.GENPOMASID,EX.COMPANYID ENAME, 
    A.DOCID,A.DOCDATE,'GENPOMAS' TNAME,A.CREATEDBY, C.PARTYID,'Purchase Order.' SNAME,'t'||A.TSID TRANSID,
    A.TOTQTY,A.APPLEVEL + 1 APPLEV, A.NET,D.TRIMNAME,A.DOCMAXNO, A.FINYEAR,AXPATTACHMENTPATH || 'genpo' || 
    A.GENPOMASID || '.pdf' POATT, CASE WHEN AXPATTACHMENTPATH IS NOT NULL AND A.POPDF1 IS NOT NULL THEN 'Y' 
    ELSE 'N' END AS HASFILE, A.REMARKS,A.POAGT, A.BUDTYPE, A.POPDF1 FROM GENPOMAS A, COMPMAS EX,PARTYMAS C,
    (SELECT X.GENPOMASID, WM_CONCAT ( DISTINCT Y.ITEM || '  RATE :-  ' || X.RATE ) TRIMNAME FROM GENPODET X, 
    ITEMMASTER Y WHERE X.ITEMNAME = Y.ITEMMASTERID AND X.GENPOMASID NOT IN (10306000000653, 11099000000431) 
    GROUP BY  X.GENPOMASID   
    UNION
    SELECT X.GENPOMASID, '---' FROM GENPODET X WHERE X.GENPOMASID IN (10306000000653, 11099000000431) GROUP 
    BY X.GENPOMASID) D, (SELECT DISTINCT APPLEVEL, C.COMPANYID FROM APPRMAS A,APPRDET B,COMPMAS C WHERE 
    A.APPRMASID = B.APPRMASID AND TSNAME = 'Purchase Order New.'  AND B.APPUSER = :UNAM  AND A.ENAME = C.COMPMASID) 
    E WHERE  A.COMP = EX.COMPMASID AND A.CANCEL = 'F' AND A.PARTYID = C.PARTYMASID AND A.GENPOMASID = D.GENPOMASID 
    AND A.REQUEST = 'T' AND EX.COMPANYID = E.COMPANYID AND ((A.APPLEVEL + 1 = E.APPLEVEL AND A.STATUS = 'Requested') 
    OR (A.STATUS = 'Rejected' AND A.APPLEVEL = E.APPLEVEL))) A,(SELECT B.COMPANYID ENAME,A.TSNAME, A.MAXLEVEL 
    FROM APPRMAS A,COMPMAS B WHERE     A.ENAME = B.COMPMASID AND TSNAME = 'Purchase Order New.') B WHERE 
    A.ENAME = B.ENAME ORDER BY A.ENAME, FINYEAR, DOCMAXNO`
